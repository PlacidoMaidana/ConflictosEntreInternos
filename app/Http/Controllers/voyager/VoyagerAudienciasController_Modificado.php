...
class VoyagerAudienciasController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    {        
       ...
       //! $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        //? MODIFICANDO EL CONTROLADOR PARA AGREGAR LA TABLA INTERNOS RELACIONADA A LA AUDIENCIA
        $additionalSlug = 'internos'; 
        ...
        //Agregando usuarios a las cabeceras de la tabla
        $usuarios = 'users'; 
        $usuariosDataType = Voyager::model('DataType')->where('slug', '=', $usuarios)->first();
        $Cabeceras_de_la_tabla = $dataTypeBrowseRows->merge($usuariosDataType->browseRows);

        // echo $dataTypeBrowseRows->toJson();die();
        // foreach ($Cabeceras_de_la_tabla as $key => $value) {
        ...
        if (strlen($dataType->model_name) != 0) {
          ...
          if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
            ...
            //? en este lugar realizamos la consulta deseada para la tabla
             /*  $query = $model::select('*')
             ->join("internos as int","audiencias.interno","=","int.id");
             */
            ...
            // Si no es administrador u operador filtrar solo por el usuario logeado
            ...
            if ($orderBy && in_array($orderBy, $dataType->fields())) {
                ...
                $dataTypeContent = call_user_func([$query->latest("audiencias.created_at"), $getter]);
                // $dataTypeContent = call_user_func([$query->latest("aud.created_at"), $getter]);
            } else {
                $dataTypeContent = call_user_func([$query->orderBy($model->getKeyName(), 'DESC'), $getter]);
            }
         //? se ordena la tabla por apellido
         //  $dataTypeContent=$query->orderBy('apellido', 'desc')->get();
        ...
        //!esto no lo usare por ahora es para ir a la vista pelada de audiencias
        //  return view('audiencias', ['brows' => $query]);
